...
# parser.add_argument('--RsltPath',type=str,help='Result path',default='../Results/FBMS/xxx')
parser.add_argument('--RsltPath',type=str,help='Result path',default='/home/xuxun/Dropbox/GitHub/LearnSubspaceMoSeg/Results/FBMS/SubspaceNet50_L2Loss_fixedLen-10_HiddenDim-256_2022-07-01_17-35-17')
...
for seq_name in seqlist_te:
    ...
    print('seq-{:s} P-{:.2f} R-{:.2f} Fm-{:.2f}'.format(seq_name, avgP, avgR, avgF))

FmeasureAvg_my = 2. * np.mean(Precision_my) * np.mean(Recall_my) / (np.mean(Precision_my) + np.mean(Recall_my))

print('Average Precision {:.2f}'.format(100 * np.mean(Precision_my)))
print('Average Recall {:.2f}'.format(100 * np.mean(Recall_my)))
print('Average Fmeasure {:.2f}'.format(100 * FmeasureAvg_my))